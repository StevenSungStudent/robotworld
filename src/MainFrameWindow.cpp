...
namespace Application {
	...
	MainFrameWindow::MainFrameWindow(const std::string &aTitle) :
			wxFrame(nullptr, wxID_ANY, aTitle, wxDefaultPosition, wxSize(1200, 600)), clientPanel(nullptr), menuBar(nullptr), splitterWindow(nullptr), lhsPanel(nullptr), robotWorldCanvas(nullptr), rhsPanel(nullptr), logTextCtrl(nullptr), logDestination(nullptr), configPanel(nullptr), drawOpenSetCheckbox(nullptr), speedSpinCtrl(nullptr), worldNumber(nullptr), buttonPanel(nullptr) {
		initialise();
	}
	/**
	 *
	...
	void MainFrameWindow::initialise() {
		...
		Bind( wxEVT_COMMAND_MENU_SELECTED, [this](wxCommandEvent &anEvent) {
			this->OnQuit(anEvent);
		},
		ID_QUIT);
		Bind( wxEVT_COMMAND_MENU_SELECTED, [this](wxCommandEvent &anEvent) {
		...
		Base::Trace::setTraceFunction(std::make_unique<
				Application::WidgetTraceFunction>(logTextCtrl));
	}
	/**
	 *
	 */
	...
	wxPanel* MainFrameWindow::initialiseClientPanel() {
		...
		sizer->Add(initialiseSplitterWindow(), wxGBPosition(1, 1), wxGBSpan(1, 1), wxGROW);
	...
	wxPanel* MainFrameWindow::initialiseLhsPanel() {
		...
		sizer->Add(robotWorldCanvas = new View::RobotWorldCanvas(lhsPanel), wxGBPosition(1, 1), wxGBSpan(1, 1), wxSHRINK);
	...
	wxPanel* MainFrameWindow::initialiseRhsPanel() {
		...
		sizer->Add(logPanel = initialiseLogPanel(), wxGBPosition(2, 1), wxGBSpan(1, 1), wxGROW);
		sizer->Add(configPanel = initialiseConfigPanel(), wxGBPosition(4, 1), wxGBSpan(1, 1), wxGROW);
		sizer->Add(buttonPanel = initialiseButtonPanel(), wxGBPosition(6, 1), wxGBSpan(1, 1), wxGROW);
		sizer->AddGrowableCol(1);

		sizer->Add(5, 5, wxGBPosition(7, 2), wxGBSpan(1, 1), wxGROW);
	...
	wxPanel* MainFrameWindow::initialiseLogPanel() {
		...
		sizer->Add(5, 5, wxGBPosition(0, 0), wxGBSpan(1, 1), wxSHRINK);

		std::array<std::string, 3> choicesArray { "Window", "StdOut", "File" };

		sizer->Add(logDestination = makeRadiobox(panel, choicesArray, [this](wxCommandEvent &event) {
		...
		"Log destination",
		wxRA_SPECIFY_COLS), wxGBPosition(1, 1), wxGBSpan(1, 1), wxALIGN_CENTER);

		sizer->Add(logTextCtrl = new LogTextCtrl(panel, wxID_ANY, wxTE_MULTILINE | wxTE_DONTWRAP), wxGBPosition(2, 1), wxGBSpan(1, 1), wxGROW);
		sizer->AddGrowableRow(2);
		sizer->AddGrowableCol(1);
		logTextCtrl->SetMinSize(wxSize(500, 300));

		sizer->Add(makeButton(panel, "Clear log window", [this](wxCommandEvent& /*anEvent*/) {
			logTextCtrl->Clear();
		}),
		wxGBPosition(3, 1), wxGBSpan(1, 1), wxGROW | wxALIGN_CENTER);

		sizer->Add(5, 5, wxGBPosition(4, 2), wxGBSpan(1, 1), wxSHRINK);
	...
	wxPanel* MainFrameWindow::initialiseConfigPanel() {
		...
		sizer->Add(drawOpenSetCheckbox = Application::makeCheckbox(panel, "Draw OpenSet", [this](wxCommandEvent &event) {
			this->OnDrawOpenSet(event);
		}),
		wxGBPosition(1, 1), wxGBSpan(1, 1), wxSHRINK | wxALIGN_CENTRE);

		/////// Speed
		sizer->Add(new wxStaticText(panel, wxID_ANY, "Speed"), wxGBPosition(2, 1), wxGBSpan(1, 1), wxSHRINK | wxALIGN_CENTER);
		sizer->Add(speedSpinCtrl = new wxSpinCtrl(panel, wxID_ANY), wxGBPosition(2, 2), wxGBSpan(1, 1), wxSHRINK | wxALIGN_CENTER);
		speedSpinCtrl->SetValue(static_cast<int>(10));
		speedSpinCtrl->Bind(wxEVT_COMMAND_SPINCTRL_UPDATED, [this](wxCommandEvent &event) {
			this->OnSpeedSpinCtrlUpdate(event);
		});

		std::array<std::string, 3> choicesArray { "1 Default world", "2 Student world 1", "3 Student world 2" };

		sizer->Add(worldNumber = makeRadiobox(panel, choicesArray, [this](wxCommandEvent &event) {
			wxRadioBox *radiobox = dynamic_cast<wxRadioBox*>(event.GetEventObject());
			if (radiobox) {
				switch (radiobox->GetSelection()) {
				case 0: {
		...
		"World number",
		wxRA_SPECIFY_ROWS), wxGBPosition(3, 1), wxGBSpan(1, 1), wxSHRINK | wxALIGN_CENTER);
		sizer->AddGrowableRow(3);
		sizer->AddGrowableCol(1);

		sizer->Add(5, 5, wxGBPosition(4, 3), wxGBSpan(1, 1), wxGROW);
	...
	wxPanel* MainFrameWindow::initialiseButtonPanel() {
		...
		sizer->Add(makeButton(panel, "Populate", [this](wxCommandEvent &anEvent) {
		...
		sizer->Add(makeButton(panel, "Start robot", [this](wxCommandEvent &anEvent) {
			this->OnStartRobot(anEvent);
		}),
		wxGBPosition(3, 1), wxGBSpan(1, 1), wxGROW);
		sizer->Add(makeButton(panel, "Stop robot", [this](wxCommandEvent &anEvent) {
			this->OnStopRobot(anEvent);
		}),
		wxGBPosition(3, 2), wxGBSpan(1, 1), wxGROW);
		sizer->Add(makeButton(panel, "Start listening", [this](wxCommandEvent &anEvent) {
			this->OnStartListening(anEvent);
		}),
		wxGBPosition(5, 1), wxGBSpan(1, 1), wxGROW);
		sizer->Add(makeButton(panel, "Send message", [this](wxCommandEvent &anEvent) {
			this->OnSendMessage(anEvent);
		}),
		wxGBPosition(5, 2), wxGBSpan(1, 1), wxGROW);
		sizer->Add(makeButton(panel, "Stop listening", [this](wxCommandEvent &anEvent) {
			this->OnStopListening(anEvent);
		}),
		wxGBPosition(5, 3), wxGBSpan(1, 1), wxGROW);
		sizer->Add(makeButton(panel, "Display kalman", [this](wxCommandEvent &anEvent) {
			this->OnDisplayKalman(anEvent);
		}),
		wxGBPosition(7, 1), wxGBSpan(1, 1), wxGROW);
		sizer->Add(makeButton(panel, "Hide kalman", [this](wxCommandEvent &anEvent) {
			this->OnHideKalman(anEvent);
		}),
		wxGBPosition(7, 2), wxGBSpan(1, 1), wxGROW);
		sizer->Add(makeButton(panel, "Display particle filter", [this](wxCommandEvent &anEvent) {
			this->OnDisplayParticleFilter(anEvent);
		}),
		wxGBPosition(9, 1), wxGBSpan(1, 1), wxGROW);
		sizer->Add(makeButton(panel, "Hide particle filter", [this](wxCommandEvent &anEvent) {
			this->OnHideParticleFilter(anEvent);
		}),
		wxGBPosition(9, 2), wxGBSpan(1, 1), wxGROW);
