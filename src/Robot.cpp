...
namespace Model {
	...
	Robot::Robot(const std::string &aName, const wxPoint &aPosition) :
			name(aName), size(wxDefaultSize), position(aPosition), front(0, 0), speed(0.0), acting(false), driving(false), communicating(false) {
		std::shared_ptr<AbstractSensor> laserSensor = std::make_shared<
		...
		std::shared_ptr<AbstractSensor> odometerCompass = std::make_shared<
				OdometerCompass>(*this);
	 ...
	 *
	 */
	void Robot::setPosition(const wxPoint &aPosition, bool aNotifyObservers /*= true*/) {
		position = aPosition;
		if (aNotifyObservers == true) {
			notifyObservers();
		}
...
// cppcheck-suppress unusedFunction
	...
	void Robot::startCommunicating() {
		if (!communicating) {
			...
			server = std::make_shared<Messaging::Server>(static_cast<unsigned short>(std::stoi(localPort)), toPtr<
					Robot>());
	...
	void Robot::stopCommunicating() {
		if (communicating) {
			...
			Messaging::Client c1ient("localhost", static_cast<unsigned short>(std::stoi(localPort)), toPtr<
					Robot>());
	...
	wxPoint Robot::getFrontLeft() const {
		...
		wxPoint frontLeft(static_cast<int>((originalFrontLeft.x - position.x) * std::cos(angle) - (originalFrontLeft.y - position.y) * std::sin(angle) + position.x), static_cast<int>((originalFrontLeft.y - position.y) * std::cos(angle) + (originalFrontLeft.x - position.x) * std::sin(angle) + position.y));
	...
	wxPoint Robot::getFrontRight() const {
		...
		wxPoint frontRight(static_cast<int>((originalFrontRight.x - position.x) * std::cos(angle) - (originalFrontRight.y - position.y) * std::sin(angle) + position.x), static_cast<int>((originalFrontRight.y - position.y) * std::cos(angle) + (originalFrontRight.x - position.x) * std::sin(angle) + position.y));
	...
	wxPoint Robot::getBackLeft() const {
		...
		wxPoint backLeft(static_cast<int>((originalBackLeft.x - position.x) * std::cos(angle) - (originalBackLeft.y - position.y) * std::sin(angle) + position.x), static_cast<int>((originalBackLeft.y - position.y) * std::cos(angle) + (originalBackLeft.x - position.x) * std::sin(angle) + position.y));
	...
	wxPoint Robot::getBackRight() const {
		...
		wxPoint backRight(static_cast<int>((originalBackRight.x - position.x) * std::cos(angle) - (originalBackRight.y - position.y) * std::sin(angle) + position.x), static_cast<int>((originalBackRight.y - position.y) * std::cos(angle) + (originalBackRight.x - position.x) * std::sin(angle) + position.y));
	...
	void Robot::calculateBelieve(OrientationPercept *orientation) {
		...
		Matrix<double, 2, 2> I { { 1.0, 0.0 }, { 0.0, 1.0 } };

		Matrix<double, 2, 1> measurement { stateVector.at(0, 0) + std::cos(orientation->angle) * orientation->distance, stateVector.at(1, 0) + std::sin(orientation->angle) * orientation->distance };
